 ...
 */
/** @module sul/protocol/minecraft333/serverbound */
...
const Serverbound = {
	...
	CraftingGrid: class extends Buffer {
		...
		encode() {
			...
			this.writeBigEndianShort(this.returnEntry.length); for(var dhc5zrc5 in this.returnEntry){ this.writeBytes(this.returnEntry[dhc5zrc5].encode()); }
			this.writeBigEndianShort(this.prepareEntry.length); for(var dhc5cvyj in this.prepareEntry){ this.writeBytes(this.prepareEntry[dhc5cvyj].encode()); }
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			...
			var aramcvdj=this.readBigEndianShort(); this.returnEntry=[]; for(var dhc5zrc5=0;dhc5zrc5<aramcvdj;dhc5zrc5++){ this.returnEntry[dhc5zrc5]=Types.Entry.fromBuffer(this._buffer); this._buffer=this.returnEntry[dhc5zrc5]._buffer; }
			var aramcjcf=this.readBigEndianShort(); this.prepareEntry=[]; for(var dhc5cvyj=0;dhc5cvyj<aramcjcf;dhc5cvyj++){ this.prepareEntry[dhc5cvyj]=Types.Entry.fromBuffer(this._buffer); this._buffer=this.prepareEntry[dhc5cvyj]._buffer; }
			return this;
		}
		...
		decode(_buffer) {
			...
			switch(this.type) {
				case 1:
					...
					break;
				case 2:
					this.bookOpened=this.readBigEndianByte()!==0;
	...
	AdvencementTab: class extends Buffer {
		...
		encode() {
			...
			this.writeVaruint(this.action);
			if(action==0){ var dhc5yi=this.encodeString(this.tab); this.writeVaruint(dhc5yi.length); this.writeBytes(dhc5yi); }
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			...
			var _id=this.readVaruint();
			this.action=this.readVaruint();
			if(action==0){ this.tab=this.decodeString(this.readBytes(this.readVaruint())); }
			return this;
		}
	...
	PlayerBlockPlacement: class extends Buffer {
		...
		encode() {
			...
			this.writeVaruint(this.hand);
			this.writeBigEndianFloat(this.cursorPosition.x); this.writeBigEndianFloat(this.cursorPosition.y); this.writeBigEndianFloat(this.cursorPosition.z);
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			...
			this.hand=this.readVaruint();
			this.cursorPosition={}; this.cursorPosition.x=this.readBigEndianFloat(); this.cursorPosition.y=this.readBigEndianFloat(); this.cursorPosition.z=this.readBigEndianFloat();
			return this;
		}
