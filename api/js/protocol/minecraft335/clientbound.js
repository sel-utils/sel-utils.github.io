 ...
 */
/** @module sul/protocol/minecraft335/clientbound */
...
const Clientbound = {
	...
	UnlockRecipes: class extends Buffer {
		...
		encode() {
			...
			this.writeVaruint(this.action);
			this.writeBigEndianByte(this.openBook?1:0);
			this.writeBigEndianByte(this.filtering?1:0);
			this.writeVaruint(this.recipes.length); for(var dhc5zncv in this.recipes){ this.writeVaruint(this.recipes[dhc5zncv]); }
			if(action==1||action==3){ this.writeVaruint(this.bookRecipes.length); for(var dhc5b9uv in this.bookRecipes){ this.writeVaruint(this.bookRecipes[dhc5b9uv]); } }
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			...
			var _id=this.readVaruint();
			this.action=this.readVaruint();
			this.openBook=this.readBigEndianByte()!==0;
			this.filtering=this.readBigEndianByte()!==0;
			var aramcvab=this.readVaruint(); this.recipes=[]; for(var dhc5zncv=0;dhc5zncv<aramcvab;dhc5zncv++){ this.recipes[dhc5zncv]=this.readVaruint(); }
			if(action==1||action==3){ var aramy9aj=this.readVaruint(); this.bookRecipes=[]; for(var dhc5b9uv=0;dhc5b9uv<aramy9aj;dhc5b9uv++){ this.bookRecipes[dhc5b9uv]=this.readVaruint(); } }
			return this;
		}
	...
	AdvancementProgress: class extends Buffer {
		...
		encode() {
			...
			this.writeBigEndianByte(this.notEmpty?1:0);
			if(notEmpty==true){ var dhc5zvdl=this.encodeString(this.identifier); this.writeVaruint(dhc5zvdl.length); this.writeBytes(dhc5zvdl); }
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			this._buffer = Array.from(_buffer);
			var _id=this.readVaruint();
			this.notEmpty=this.readBigEndianByte()!==0;
			if(notEmpty==true){ this.identifier=this.decodeString(this.readBytes(this.readVaruint())); }
			return this;
		}
